...
export async function POST(req: Request) {
  const { name, slug, id, projectId } = await req.json();
  ...
  if (!id) {
    ...
    try {
      ...
      return Response.json({ error: 'Kindly ensure that the page name or slug does not  exist.' });
    }
  } else {
    try {
...
export async function PATCH(req: Request) {
  const { name, slug, metaTitle, metaKeyWords, metaDescription, id } = await req.json();
